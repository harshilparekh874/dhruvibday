...
export default function App() {
  ...
  // Fade-in "Venice, Italy" once the scene/background has fully transitioned in.
  useEffect(() => {
    if (hasAnimationCompleted && environmentProgress > 0.98) {
      // next tick so transition reliably animates from opacity 0 -> 1
      const id = window.setTimeout(() => setShowLocationLabel(true), 50);
      return () => window.clearTimeout(id);
    }
    setShowLocationLabel(false);
  }, [hasAnimationCompleted, environmentProgress]);
    ...
    <div className="App">
      ...
      >
        <Suspense fallback={null}>
          ...
          />
          <Environment
            files={["/venice_dawn_2_4k.hdr"]}
            backgroundRotation={[0, 3.3, 0]}
            environmentRotation={[0, 3.3, 0]}
            background
            environmentIntensity={0.9 * environmentProgress}
